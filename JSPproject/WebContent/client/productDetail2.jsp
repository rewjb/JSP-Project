...
%>
	...
	<div style="margin: 0 auto;  width: 900px; position: relative; right: 370px; top:300px; " >
		...
		<form  action="productDetailToCart.jsp" name="form" method="get" style="position: relative; left: 650px; bottom: 450px;">
			...
			onclick="location.href='payment.jsp'">바로 결제</button>
<%-- 			<button type="button" class="btn btn-primary" style="width: 400px; position: relative; top: 50px; right: 170px;"  
			onclick="location.href='payment2.jsp?mid=<%=session.getAttribute("id")%>&pid=<%=pid%>&price=<%=pdto.getPrice() %>&name=<%=pdto.getName() %>'" >바로 결제</button> --%>
		</form>
